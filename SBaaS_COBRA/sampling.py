...
class cobra_sampling():
    ...
    def descriptive_statistics(self,points_I='flux'):
        ...
        INPUT:
        points_I = 'flux','metabolite','subsystem', default='flux'
        '''
        ...
        for k,v in points.items():
            ...
            n = len(points[k]);
            m,var,lb,ub = self.calculate.calculate_ave_var(points[k],confidence_I = 0.95);
            # directly calculate the 95% CI
            lb,ub = self.calculate.calculate_ciFromPoints(points[k],alpha=0.05)
            #lb,ub = self.calculate.bootstrap(points[k]['points'], num_samples=100000, statistic=np.mean, alpha=0.05)
            # calculate the min, max, median, and interquartile ranges
            min,max,median,iq_1,iq_3=self.calculate.calculate_interquartiles(points[k],iq_range_I = [25,75])
            tmp = {};
            tmp = {
                'n':n,
                'ave':m,
