...
class stage02_physiology_analysis_query(sbaas_template_query):
    ...
    def get_simulationID_analysisID_dataStage02PhysiologyAnalysis(self,analysis_id_I):
        ...
        try:
            ...
            print(e);
    def add_data_stage02_physiology_analysis(self, data_I):
        '''add rows of data_stage02_physiology_analysis'''
        if data_I:
            for d in data_I:
                try:
                    data_add = data_stage02_physiology_analysis(
                    ...
                    print(e);
            self.session.commit();
    def update_data_stage02_physiology_analysis(self,data_I):
        #TODO:
        '''update rows of data_stage02_physiology_analysis'''
        if data_I:
            for d in data_I:
                try:
                    data_update = self.session.query(data_stage02_physiology_analysis).filter(
                            data_stage02_physiology_analysis.id.like(d['id'])
                            ).update(
                            {
    ...
    def drop_dataStage03_quantification_analysis(self):
        try:
            data_stage02_physiology_analysis.__table__.drop(self.engine,True);
        except SQLAlchemyError as e:
            print(e);
    def reset_dataStage03_quantification_analysis(self,analysis_id_I = None):
        try:
            if analysis_id_I:
                reset = self.session.query(data_stage02_physiology_analysis).filter(data_stage02_physiology_analysis.analysis_id.like(analysis_id_I)).delete(synchronize_session=False);
                self.session.commit();
        except SQLAlchemyError as e:
            print(e);
